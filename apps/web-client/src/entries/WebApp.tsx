...
export default function WebApp() {
    ...
    const enumerate = async () => {
      try {
        ...
        try {
          ...
          const inputs = devices.filter(d => d.kind === 'audioinput').map((d, i) => ({ id: d.deviceId || `in-${i}`, name: d.label || `Mic ${i + 1}`, is_default: false }))
          const outputs = devices.filter(d => d.kind === 'audiooutput').map((d, i) => ({ id: d.deviceId || `out-${i}`, name: d.label || `Speaker ${i + 1}`, is_default: false }))
  ...
  const startMicTest = async () => {
    ...
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ audio: selectedInputId ? { deviceId: { exact: selectedInputId } as any } : true })
  ...
  const playSpeakerTest = async () => {
    try {
      ...
      if ((audioEl as any).setSinkId && selectedOutputId) {
        try { await (audioEl as any).setSinkId(selectedOutputId) } catch (err) { addLog(`setSinkId failed: ${err}`) }
      }
      ...
      <main className="container mx-auto px-4 py-6 pb-20">
        <Tabs defaultValue="record" className="w-full">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="record" className="text-xs">Audio</TabsTrigger>
          ...
          <TabsContent value="record" className="mt-4">
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center"><Mic className="mr-2 h-5 w-5" />Audio Devices</CardTitle>
                <CardDescription>Enumerate devices and test connectivity in browser</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-sm font-medium">Input Device</label>
                    <Select value={selectedInputId} onValueChange={setSelectedInputId} disabled={!browserDevices || browserDevices.inputs.length === 0}>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={selectedOutputId} onValueChange={setSelectedOutputId} disabled={!browserDevices || browserDevices.outputs.length === 0}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      <Button onClick={micTestRunning ? stopMicTest : startMicTest} size="sm">{micTestRunning ? 'Stop' : 'Start'}</Button>
                      <div className="flex-1 h-2 bg-muted rounded">
                        <div className="h-2 bg-green-500 rounded" style={{ width: `${Math.min(100, Math.round(micLevel * 200))}%` }} />
                      </div>
                    </div>
          ...
          <TabsContent value="logs" className="mt-4">
            <Card>
              <CardHeader><CardTitle>Logs</CardTitle><CardDescription>Runtime logs for web client UI</CardDescription></CardHeader>
              <CardContent>
                <div className="space-y-2">
                  <div className="flex justify-end"><Button size="sm" variant="secondary" onClick={clearLogs}>Clear</Button></div>
